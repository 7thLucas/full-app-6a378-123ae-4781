...
export default function FollowersPage() {
  ...
  useEffect(() => {
    fetch(`/api/social/followers/${params.userId}`)
      .then(r => r.json())
  ...
  return (
    <div className="min-h-screen bg-background">
      <header className="sticky top-0 z-40 bg-navbar border-b border-border px-4 flex items-center gap-3 h-14">
        ...
        <h1 className="font-semibold text-foreground">Followers</h1>
      </header>
      <main className="max-w-lg mx-auto pb-20">
        ...
        {!loading && followers.length === 0 && (
          <div className="text-center py-16 text-muted-foreground">No followers yet.</div>
        )}
